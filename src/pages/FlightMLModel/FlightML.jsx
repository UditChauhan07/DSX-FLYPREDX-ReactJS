...
const FlightML = () => {
  ...
  // Call your ML-model API on Predict
  const handlePredict = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/predict_delay`, {
        method: "POST",
      ...
      setModelAccuracy(data.data_accuracy);
    } catch (err) {
      console.error(err);
                  ...
                  predictedDelay !== null && (
                    <div className="bg-yellow-50 dark:bg-yellow-900 border-l-4 border-yellow-400 dark:border-yellow-600 p-6 rounded-lg">
                      <h3 className="text-lg font-semibold text-yellow-700 dark:text-yellow-300 mb-2">
                        🔮 Model Prediction by ML Model (Github)
                      </h3>
