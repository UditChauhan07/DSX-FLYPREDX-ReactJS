...
const FlightShivam = () => {
  ...
  function formatToDayMonth(dateStr) {
    const parsedDate = new Date(dateStr + " 2024"); // Add dummy year to ensure it's parseable
    if (isNaN(parsedDate)) return "Invalid Date";

    const day = parsedDate.getDate();
    const month = parsedDate.toLocaleString("default", { month: "short" }); // 'May', 'Jun', etc.

    return `${day} ${month}`; // e.g., "20 May"
  }

  // Call your Shivam-AI-model API on Predict
  const handlePredict = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/predict_by_flight_number`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          flight_number: flightNumber.toString(),
          date: formatToDayMonth(flightDetails?.date),
        }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      console.log("ress", data);
      setPredictedDelay(data.predicted_delay_arrival);
      setModelAccuracy(data.model_accuracy_percent);
    } catch (err) {
      console.error(err);
                  ...
                  predictedDelay !== null && (
                    <div className="bg-yellow-50 dark:bg-yellow-900 border-l-4 border-yellow-400 dark:border-yellow-600 p-6 rounded-lg">
                      <h3 className="text-lg font-semibold text-yellow-700 dark:text-yellow-300 mb-2">
                        🔮 Model Prediction by ML Model (DesignersX)
                      </h3>
